...
Pod::Spec.new do |s|
  ...
  framework_path = File.join(__dir__, "ios", "TencentOpenAPI.framework")
  ...
  unless File.exist?(framework_path) && File.exist?(bundle_path)
    raise <<~MSG
      ...
      [md-native-qq] 未找到 TencentOpenAPI.framework / TencentOpenApi_IOS_Bundle.bundle。
  ...
  #    新 linker ld_prime（Xcode 15+ 默认）在处理 TencentOpenAPI.framework 的
  ...
  #    在腾讯发布新工具链编出来的 framework 之前只能 workaround。
